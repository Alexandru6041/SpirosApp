...
void sha256(const uint8_t *data, size_t len, uint8_t out[32]) {
    ///Padding the message to fit in the first multiple of 512 greater or equal than the length of the message;
    uint32_t h[8];
    for(int i = 0; i < 8; i++) {
        h[i] = H0[i];
    }
    ...
    size_t padded_length = len + 1 + 8;
    while(padded_length % 64 != 0){
        padded_length++;
    }

    uint8_t *message = calloc(padded_length, 1);
    if(!message)
        return;
    
    memcpy(message, data, len);
    message[len] = 0x80; //appending the 1 bit

    for(int i = 0; i < 8; i++ )
        message[padded_length - i - 1] = (uint8_t) (bit_len >> (8 * i));

    
    for(size_t block = 0; block < padded_length; block += 64) {
        const uint8_t *p = message + block;
        uint32_t w[64];
        ...
        for(int i = 0; i < 64; i++) {
            uint32_t t1 = hh + big_sigma1(e) + choose(e, f, g) + K[i] + w[i];
